...
mygraph.Nodes.Betweenness = num2str(node_betweenness_faster(adjacency(mygraph))');
%mygraph.Nodes.Eigen = num2str(eig(adjacency(graph)));
...
bar(yvals)
Interstaradj = [ones(10,25); ones(15,10) zeros(15,15)];
Interstaradj(logical(eye(size(Interstaradj)))) = 0;
interstar = graph(Interstaradj);
bar(sort(degree(interstar)));
hold on
bar(sort(degree(mygraph)),'FaceColor',[0 0.7 0.7]);
hold off